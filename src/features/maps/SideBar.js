...
export default function PermanentDrawerLeft() {
  ...
  const handleOpenInventory = (index) => {
    index === 1 ? setOpenInventory((prev) => !prev) : (index === 2) ? setOpenReports((prev) => !prev) : console.log()
  }
          ...
          {pat.map((text, index) => (
            <>
              ...
              >
                ...
                {index === 2 && (openInventory ? <ExpandLess /> : <ExpandMore />)}
              </ListItem>
              {index === 1 && inventoryCollapse.map((text,index) => (
                <Collapse in={openInventory} timeout="auto" unmountOnExit sx={{backgroundColor:"#EBEBEB"}}>
              ...
              {index === 2 && reportCollapse.map((text,index) => (
                <Collapse in={openReports} timeout="auto" unmountOnExit sx={{backgroundColor:"#EBEBEB"}}>
                  <List component="div" disablePadding>
                    <ListItemButton
                      onClick={(e) => {
                        e.stopPropagation();
